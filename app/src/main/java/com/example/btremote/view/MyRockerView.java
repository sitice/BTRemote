...
public class MyRockerView extends View {
    ...
    private final boolean hasCall = false;
    ...
    public MyRockerView(Context context, Drawable areaBackground, Drawable rockerBackground) {
        super(context);

        // 获取自定义属性
        initAttribute( areaBackground, rockerBackground);

        isInEditMode();

        // 移动区域画笔
        mAreaBackgroundPaint = new Paint();
        mAreaBackgroundPaint.setAntiAlias(true);//抗锯齿
     ...
     *
     */
    private void initAttribute( Drawable areaBackground, Drawable rockerBackground) {
        // 可移动区域背景
    ...
    public void setDistanceLevel(int mDistanceLevel) {
        this.mDistanceLevel = mDistanceLevel;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (AREA_BACKGROUND_MODE_PIC == mAreaBackgroundMode || AREA_BACKGROUND_MODE_XML == mAreaBackgroundMode) {
            // 图片
            @SuppressLint("DrawAllocation") Rect src = new Rect(0, 0, mAreaBitmap.getWidth(), mAreaBitmap.getHeight());
            @SuppressLint("DrawAllocation") Rect dst = new Rect(mCenterPoint.x - mAreaRadius, mCenterPoint.y - mAreaRadius, mCenterPoint.x + mAreaRadius, mCenterPoint.y + mAreaRadius);
        ...
        if (ROCKER_BACKGROUND_MODE_PIC == mRockerBackgroundMode || ROCKER_BACKGROUND_MODE_XML == mRockerBackgroundMode) {
            // 图片
            @SuppressLint("DrawAllocation") Rect src = new Rect(0, 0, mRockerBitmap.getWidth(), mRockerBitmap.getHeight());
            @SuppressLint("DrawAllocation") Rect dst = new Rect(mRockerPosition.x - mRockerRadius, mRockerPosition.y - mRockerRadius, mRockerPosition.x + mRockerRadius, mRockerPosition.y + mRockerRadius);
    ...
    public void setOnShakeListener(@NotNull OnShakeListener onShakeListener) {
