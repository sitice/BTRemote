...
class LineChart(
    ...
    private lateinit var model: WaveViewModel

    fun setViewModel(viewModel: WaveViewModel) {
        model = viewModel
    }

    private val timerTask = object : TimerTask() {
        override fun run() {
            model.dfProtocolDataFlow.value.forEach { wave ->
                wave.line.apply {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        model.dfProtocolDataFlow.value.forEach { wave ->
            if (wave.visible.value)
            wavePaint.color = wave.color.value
            wave.line.apply {
                if (points.isNotEmpty()) {
                    if (endDrawPoint == null) {
                        endDrawPoint = points.size - 1
